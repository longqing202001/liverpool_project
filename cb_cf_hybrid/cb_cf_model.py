...
def evaluate_model(model, user_valid_input, item_valid_input, valid_item_feature_input, valid_labels, topK):
    """
    Evaluate the performance (Hit_Ratio, NDCG) of top-K recommendation
    Return: score of each test rating.
    """  
    hits, ndcgs = [],[]
    predictions = model.predict([user_valid_input, item_valid_input, valid_item_feature_input], batch_size=args.batch_size, verbose=0)
...
def get_model(num_users, num_items, item_features, latent_dim, regs=[1e-6,1e-6]):
    ### define placeholder.
...
# def get_model2(num_users, num_items, item_features, latent_dim, regs=[0,0]):
#     user = Input(shape=(1,))
#     u = Embedding(num_users, latent_dim, embeddings_initializer='he_normal',
#                   embeddings_regularizer=l2(1e-6))(user)
#     u = Reshape((latent_dim,))(u)
    
#     item = Input(shape=(1,))
#     m = Embedding(num_items, latent_dim, embeddings_initializer='he_normal',
#                   embeddings_regularizer=l2(1e-6))(item)
#     m = Reshape((latent_dim,))(m)
#     x = Dot(axes=1)([u, m])
#     model = Model(inputs=[user, item, item_feature_input], outputs=x)
#     return model


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='content based and collaborative filtering recommendations')
    ...
    parser.add_argument('--train_loss_path', type=str, default='data/preprocessed_data/cf+cb_loss.png')  
    parser.add_argument('--evaluation_path', type=str, default='data/preprocessed_data/cf+cb_evaluation.png')    
    ...
    model_out_file = args.base_path+args.checkpoint_path+'CB_CF_%d_%d.h5' %(args.latent_dim, time())
    model = get_model(num_users, num_items, item_features, args.latent_dim, regs=[0,0])
    ...
    for epoch in range(args.epochs):
        with tqdm(total=len(item_train_input)) as pbar:
            ...
            while (i<count):
                ...
                #slice_user_train_input = to_categorical(slice_item_train_input, num_classes=num_users)

                slice_item_train_input = item_train_input[start_index:end_index]
                #slice_item_train_input = to_categorical(slice_item_train_input, num_classes=num_items) 
                ...
                t1 = time()
                # Training
                hist = model.fit([slice_user_train_input, slice_item_train_input,train_item_feature_input], #input
        ...
        (hits, ndcgs) = evaluate_model(model, user_valid_input, item_valid_input, valid_item_feature_input, valid_targetlist, args.topK)
    ...
    with open(args.base_path+'result/cf_cb_train_losses.pkl', 'wb') as f:
        pickle.dump(train_losses, f)   

    with open(args.base_path+'result/cf_cb_hr_list.pkl', 'wb') as f:
        pickle.dump(hr_list, f) 

    with open(args.base_path+'result/cf_cb_ndcg_list.pkl', 'wb') as f:
        pickle.dump(ndcg_list, f) 

    plt.plot(train_losses)
    plt.title('CF+CB Model Loss')
    plt.ylabel('Loss')
    plt.xlabel('Epoch')
    plt.legend(['train'], loc='upper right')
    plt.savefig(args.base_path+args.train_loss_path)

    plt.clf()
    plt.plot(hr_list)
    plt.plot(ndcg_list)
    plt.title('CF+CB Model Evaluation')
    plt.ylabel('Metrics')
    plt.xlabel('Epoch')
    plt.legend(['HR@50',"NDCG@50"], loc='upper right')
    plt.savefig(args.base_path+args.evaluation_path)